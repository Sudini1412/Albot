...
def dynamic_load(bot):
    ...
    mock_load.bot.extensions = {
    'albot.cogs.dynamic_load': None,
    'cogs.test_cog': None
    }
...
def test_reload_cog(dynamic_load):
    ...
    dynamic_load.bot.reload_extension.assert_has_calls([
        call('cogs.test_cog')
    ])

    assert dynamic_load._reload_cog('cogs.new_cog') == 'Loaded new cog `cogs.new_cog`'
    dynamic_load.bot.load_extension.assert_has_calls([
        call('cogs.new_cog')
    ])
    ...
    assert dynamic_load._reload_cog('cogs.test_cog') == '`cogs.test_cog` raised exception: ```\ntest_exception\n```'
...
def test_fmt_cog_list(dynamic_load):
    ret = dynamic_load._fmt_cog_list(
        ['cogs.new_cog', 'cogs.test_cog']
    )
    assert ret == "```\n- cogs.new_cog\n- cogs.test_cog\n```"
